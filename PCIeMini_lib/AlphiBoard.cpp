...
//
/** @file AlphiBoard.cpp
* @brief Implementation of the base PCIe board class with Jungo driver and Altera PCIe hardware
*/
// Maintenance Log
//---------------------------------------------------------------------
...
HRESULT AlphiBoard::Open(int brdNbr)
{
	...
	// check if the Jungo Library is initialized
	if (WD_STATUS_SUCCESS != libStatus) {
		if (verbose) {
			cout << "Jungo library init failed with error: " << getAlphiErrorMsg(libStatus) << endl;
		}
...
DWORD AlphiBoard::DeviceOpen()
{
	...
	for (uint32_t i = 0; i < pDev->dwNumAddrSpaces; i++) {
		...
		if (desc->dwAddrSpace == 3 && desc->fIsMemory) {
			bar3.Address = (void*)desc->pUserDirectMemAddr;
			bar3.Length = desc->qwBytes;
		}
...
DWORD AlphiBoard::printAddressSpace(WDC_ADDR_DESC* desc)
{
	...
	cout << "Item index = " << desc->dwItemIndex << endl;            /* Index of address space in the pDev->cardReg.Card.Item array */
	if (desc->fIsMemory)
	{
