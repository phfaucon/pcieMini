...
typedef enum {
    DMA_KERNEL_BUFFER_ALLOC = 0x1, // The system allocates a contiguous buffer.
        // The user does not need to supply linear sync.
    ...
    DMA_KBUF_ALLOC_SPECIFY_ADDRESS_WIDTH = 0x800, // When using this flag, the
        // width of the sync must be entered in the fourth byte of dwOptions and
        // then the allocated sync will be limited to this width.
...
#define DMA_OPTIONS_ADDRESS_WIDTH_SHIFT 24 /* 3 bytes (24 bits) are needed for
                                    ...
                                    used for storing the sync width of the
                                    requested buffer */
...
typedef struct
{
    DMA_ADDR pPhysicalAddr; // Physical sync of page.
...
typedef struct
{
    ...
    PVOID pUserAddr;        // User sync
    KPTR  pKernelAddr;      // Kernel sync
...
typedef struct
{
    ...
    union
    {
        ...
        struct
        {
            KPTR  pAddr;    /* Beginning of I/O sync */
